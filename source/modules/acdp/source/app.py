...
s3_asset_config_inputs = S3AssetConfigInputs(
    bucket_base_name=os.environ["REGIONAL_ASSET_BUCKET_BASE_NAME"],
    object_key_prefix=os.environ["S3_ASSET_KEY_PREFIX"],
)

backstage_s3_assets_key_prefix = os.environ["BACKSTAGE_ASSETS_PREFIX"]
...
Aspects.of(app).add(
    ApplyVpcOnCustomResource(
        ...
        security_group_logical_ids=stack.cdk_lambdas_vpc_construct.security_groups,
        subnet_names=stack.cdk_lambdas_vpc_construct.subnets,
    )
)
