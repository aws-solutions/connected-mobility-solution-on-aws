...
import { paletteI18nStrings, getTextFilterCounterText } from "@/i18n-strings";
...
export default function Palette({ items: allItems }: PaletteProps) {
  const { items, filterProps, collectionProps, filteredItemsCount, actions } =
    useCollection(allItems, {
      filtering: {
        filteringFunction: (item, filteringText) =>
          compareStrings(item.data.title, filteringText) ||
          compareStrings(item.data.description, filteringText),
        empty: (
          <EmptyState
      ...
      sorting: {
        defaultState: {
          sortingColumn: {
            sortingComparator: (a, b) => (a.data.title > b.data.title ? 1 : -1),
          },
        },
      },
    });
