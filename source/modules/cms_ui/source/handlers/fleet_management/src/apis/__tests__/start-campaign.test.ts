...
describe("startCampaign", () => {
  ...
  it("should successfully start a campaign", async () => {
    ...
    // Call the function
    const result = await startCampaign(mockInput);
  ...
  it("should throw CampaignNotFound when campaign doesn't exist", async () => {
    ...
    await expect(startCampaign(mockInput)).rejects.toThrow(CampaignNotFound);
    await expect(startCampaign(mockInput)).rejects.toMatchObject({
  ...
  it("should throw CampaignBeingModified when campaign is being modified", async () => {
    ...
    // Call and verify exception
    await expect(startCampaign(mockInput)).rejects.toThrow(
      CampaignBeingModified,
    );
    await expect(startCampaign(mockInput)).rejects.toMatchObject({
      campaignName: mockInput.name,
  ...
  it("should propagate unknown errors", async () => {
    ...
    // Call and verify exception
    await expect(startCampaign(mockInput)).rejects.toThrow("Unknown error");
  });
});
