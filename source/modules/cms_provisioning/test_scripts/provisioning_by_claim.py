...
TEMPLATE_NAME = f"{APP_UNIQUE_ID}--provisioning-template"
# Root CA URL
...
IOT_CLAIM_CERTS_NAME = f"{APP_UNIQUE_ID}/cms-provisioning/provisioning-credentials"
...
def get_iot_client() -> IoTClient:
    return boto3.client("iot", region_name=os.environ["AWS_REGION"])
...
class ProvisioningByClaim:  # pylint: disable=too-many-instance-attributes
    ...
    def fetch_claim_credentials(self) -> ClaimCredentials:
        ...
        cache = SecretCache(config=cache_config, client=get_secretsmanager_client())
    ...
    def send_vehicle_connection_request(self) -> None:
        mqtt_connection = mqtt_connection_builder.mtls_from_bytes(
            ...
            port=REMOTE_PORT,
            cert_bytes=(
                self.provisioned_credentials.certificate_pem
                if self.provisioned_credentials is not None
                else b""
            ),
            pri_key_bytes=(
                self.provisioned_credentials.private_key
                if self.provisioned_credentials is not None
                else b""
            ),
            ca_bytes=self.claim_credentials.ca_certificate,
        ...
        message = {
            "vin": self.vin,
            "certificate_id": (
                self.provisioned_credentials.certificate_id
                if self.provisioned_credentials is not None
                else ""
            ),
        }
