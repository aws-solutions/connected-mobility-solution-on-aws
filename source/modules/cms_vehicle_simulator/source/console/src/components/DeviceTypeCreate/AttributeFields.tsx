...
export default function AttributeFields(props: IProps): React.JSX.Element {
  /**
   ...
   */
  // prettier-ignore
  const createFormControlItem = (id: keyof IAttribute) => { // NOSONAR
    const attrType = props.attr.type;
    let formControlOptions: FormControlProps &
      React.InputHTMLAttributes<Function> = {};
    let options: Array<string> = [];

    const getFieldOptionsForStaticAndTimeStamp = (id: string) => {
      if (id == "static") {
        return [I18n.get("false"), I18n.get("true")];
      } else {
        return [
          I18n.get("timestamp.tsformat.default"),
          I18n.get("timestamp.tsformat.unix"),
        ];
      }
    };
  ...
  return (
    <Col>
      {Object.keys(props.attr)
        .filter((word) => !["name", "type", "payload"].includes(word))
        .map((id, index) => (
          <Form.Group key={`${id}-${index}`}>
            <Form.Label>
            ...
            <Form.Control.Feedback type="invalid">
              {props.errors[id as keyof IAttribute]}
            </Form.Control.Feedback>
            <Form.Text>
              {id === "static" || id === "default"
                ? I18n.get(`${id}.description`)
                : I18n.get(
                    `${props.attr.type.toLowerCase()}.${id}.description`,
                  )}
            </Form.Text>
          </Form.Group>
        ))}
