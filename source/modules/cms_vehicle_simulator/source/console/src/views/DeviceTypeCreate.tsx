...
import { API_NAME, validateField, validateFileContents } from "../util/Utils";
...
export default function DeviceTypeCreate(props: IProps): React.JSX.Element {
  const navigate = useNavigate();
  const logger = new Logger("Device Type Create");
  const [vehicleDemoPayload, setVehicleDemoPayload] = useState<object>({});
  ...
  const [refAttrPayload, setRefAttrPayload] = useState<
  ...
  const loadVehicleDemoPayload = async () => {
    try {
      const results = await API.get(API_NAME, "/template/vehicle", {});
      setVehicleDemoPayload(results);
    } catch (err) {
  ...
  const handleSubmit = async (event: any) => {
    ...
    try {
      await API.post(API_NAME, "/device/type", { body: deviceType });
  ...
  const attributeModal = (attr: IAttribute, indicesString: string) => {
    return (
      <Modal
        show={showAttrModal === indicesString}
        onHide={() => setShowAttrModal("")}
      >
        <Modal.Header closeButton>
          <Modal.Title>{attr.name}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
  ...
  /**
   * generates sample data for an attribute
   * @param attr
   * @returns sample data
   */
  // prettier-ignore
  const generateSampleData = (attr: IAttribute) => { // NOSONAR
    if (attr.default) {
      return attr.default;
    }
    ...
    if (attr.type === "string") {
      ...
      if (attr.tsformat === "unix") {
        ...
        return moment("2018-02-15 13:50:18")
          .utc()
  ...
  const parseTableRows = (payload: IAttribute[], nesting = 0, prefix = "") => {
    return payload.map((attribute, i) => (
      <Fragment key={`${prefix}_${attribute.name}`}>
        <tr>
          <td style={{ paddingLeft: `${nesting * 2.5}vh` }}>
            {attribute.name}
          </td>
          <td>{attribute.type}</td>
          <td>{attribute.static?.toString()}</td>
          <td>
            ...
            {attribute.type !== "object"
              ? attributeModal(attribute, `${prefix}${i}`)
              : ""}
            <Button
              size="sm"
  ...
  const downloadDeviceType = () => {
    let fileData = (({ name, topic, payload }) => ({ name, topic, payload }))(
      deviceType,
    );
      ...
      <Card className="content-card">
        <Card.Title className="content-card-title">
          ...
          <Button
            ...
            className="button-theme header-button"
            onClick={() =>
              setDeviceType({ ...deviceType, ...vehicleDemoPayload })
            }
          >
          ...
          <Form.Control
            ...
            hidden
          ></Form.Control>
          <Button size="sm" className="button-theme header-button">
        ...
        <Card.Body>
          ...
          >
            ...
            <Form.Group className="form-item-spacing">
              <Form.Label htmlFor="topic">{I18n.get("topic")}</Form.Label>
              <Form.Control
                required
                type="text"
                id="topic"
            ...
            <Form.Group className="form-item-spacing">
              ...
              <Table className="form-table table-header">
                ...
                <tbody>{parseTableRows(deviceType.payload)}</tbody>
              </Table>
              <ModalForm
                refAttr={refAttrPayload}
                showModal={showAttrFormModal}
                closeModal={closeAttrFormModal}
