 ...
 */
export default function SimulationCreate(props: IPageProps): React.JSX.Element {
    ...
    switch (event.target.id) {
      ...
      case "type":
        ...
        if (type === simTypes.autoDemo) {
          simulation.sim_id = simTypes.autoDemo;
        } else {
          simulation.sim_id = "";
        }
        let devices = [{ type_id: "", name: "", amount: 1 }];
        ...
        <Card.Body>
          ...
          >
            ...
            <Form.Group>
              ...
              >
                <option value={simTypes.custom}>
                  {I18n.get("user.created")}
                </option>
                <option value={simTypes.autoDemo}>
                  {I18n.get("vehicle.demo")}
                </option>
              </Form.Control>
              <Form.Text>{I18n.get("simulation.type.description")}</Form.Text>
            </Form.Group>
            <DeviceFields
              simType={simType}
            ...
            />
            <Form.Group>
              <Form.Label htmlFor="interval">{I18n.get("interval")}</Form.Label>
              <Form.Control
                type="number"
                id="interval"
                ...
                onFocus={(event: any) => handleFieldFocus(event)}
                isInvalid={
                  !!errs.interval && showValidation.includes("interval")
                }
              ...
              <Form.Text muted>{I18n.get("interval.description")}</Form.Text>
            </Form.Group>
            <Form.Group>
              <Form.Label htmlFor="duration">{I18n.get("duration")}</Form.Label>
              <Form.Control
                type="number"
                id="duration"
                onChange={(event) => handleFormChange(event)}
                onFocus={(event: any) => handleFieldFocus(event)}
                isInvalid={
                  !!errs.duration && showValidation.includes("duration")
                }
              ...
              <Form.Text>{I18n.get("duration.description")}</Form.Text>
            </Form.Group>
