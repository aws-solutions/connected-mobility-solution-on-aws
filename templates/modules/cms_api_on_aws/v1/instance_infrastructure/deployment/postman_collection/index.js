...
const gqlData = fs.readFileSync("../../source/infrastructure/assets/graphql/schemas/vss_schema.graphql", { encoding: "UTF8" });

program
    .requiredOption('--stack-name <string>', 'Stack name to get CfnOutputs from')
    .option('--region <string>', 'Region stack is deployed in', process.env.AWS_DEFAULT_REGION)
...
async function getCfnOutputs(stackName) {
    // Get the CloudFormation stack outputs for the cms-api-on-aws-stack-dev stack.
    ...
    const graphql_endpoint_url = response.Stacks[0].Outputs.find(o => o.OutputKey.includes("graphqlendpointurl")).OutputValue;
    return {
    ...
    Converter.convert(
        ...
        (err, conversionResult) => {
            if (!conversionResult.result) {
                ...
                try {
                    fs.writeFileSync("./cms_graphql_api_postman_collection.json", JSON.stringify(collectionJSON));
