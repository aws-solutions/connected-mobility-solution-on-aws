...
vehicle_simulator_console_dir="$source_dir/console"

# Run tests for vehicle simulator front-end console application. This must be done before python testing so the cloudformation distribution
# can find the front-end asset.
npm install --no-save --prefix=$vehicle_simulator_console_dir
npm run build --prefix=$vehicle_simulator_console_dir
npm run test --prefix=$vehicle_simulator_console_dir
did_console_test_failure_occur=$?

# Check results of front-end tests
if [[ $did_console_test_failure_occur -ne 0 ]]
then
  echo "===================================================="
  echo "test FAILED for $vehicle_simulator_console_dir"
  echo "===================================================="
fi

rm -rf $vehicle_simulator_console_dir/coverage/lcov-report
# <=====UNIQUE TO VEHICLE SIMULATOR=====>
...
# Exit if either of the tests failed
if [[ $did_console_test_failure_occur -ne 0 || $did_test_failure_occur -ne 0 ]]
then
  exit 1
fi
