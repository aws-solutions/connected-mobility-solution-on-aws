...
from ...infrastructure.cms_vehicle_simulator_on_aws_stack import VSApiStack


def test_vsapi_log_group_count(vsapi_stack: VSApiStack) -> None:
    template = Template.from_stack(vsapi_stack)
    template.resource_count_is("AWS::Logs::LogGroup", 1)


def test_vsapi_role_count(vsapi_stack: VSApiStack) -> None:
    template = Template.from_stack(vsapi_stack)
    template.resource_count_is("AWS::IAM::Role", 1)


def test_vsapi_apigw_rest_api_count(
    vsapi_stack: VSApiStack,
) -> None:
    template = Template.from_stack(vsapi_stack)
...
def test_vsapi_apigw_path_count(
    ...
    template = Template.from_stack(vsapi_stack).to_json()
    paths = (
