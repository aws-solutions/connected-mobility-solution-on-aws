...
export default function Simulations(props: IPageProps): React.JSX.Element {
   ...
   */
  useEffect(() => {
    setShowAlert(simulations.length === 0);
  }, [simulations]);
  ...
  const emptySimAlert = () => {
    return (
      <Alert className="empty-alert" variant="secondary" show={showAlert}>
        <Alert.Heading>{I18n.get("no.simulations")}</Alert.Heading>
        <a href="/device/type/create">{I18n.get("create.device.type")}</a>&nbsp;
        {I18n.get("to.get.started")}.<br />
        {I18n.get("if.device.type.created")},&nbsp;
        <a href="/simulations/create">
          {I18n.get("create.simulation").toLowerCase()}
        </a>
        &nbsp;
