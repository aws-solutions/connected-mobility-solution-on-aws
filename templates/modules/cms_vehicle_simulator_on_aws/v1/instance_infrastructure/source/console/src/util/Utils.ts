 ...
 */
export function validateFileContents(contents: IDeviceType) { // NOSONAR
 ...
 * @param payload
 */
export function validatePayload(payload: IAttribute[]) { // NOSONAR
  let index = 0;
  ...
  try {
    for (index; index < payload.length; index++) {
      ...
      if (!payloadFields.includes("type")) {
        ...
        if (
          Object.keys(validateField("type", payload[index].type)).length > 0
        ) {
      ...
      let attrFields = getAttrFields(payload[index].type) as Array<
        keyof IAttribute
      >;
      ...
      attrFields.forEach((field: keyof IAttribute) => {
        ...
        //Validate fields
        let errors: IErrors<IAttribute> = validateField(
          field,
          payload[index][field]
        );
        if (Object.keys(errors).length > 0) {
 ...
 * @returns boolean
 */
function confirmValidType<K extends keyof IAttribute>(
  field: K,
  value: IAttribute[K]
): boolean {
...
export function verifyAttributeField(
  ...
  if (!confirmValidType(field, value)) {
    return {
      [field]: `${getErrors("general", "type")}, ${I18n.get("expecting")} ${AttributeTypeMap[field]}`,
    };
  }
  switch (field) {
    case "lat":
    ...
    case "long":
      ...
      break;
    case "charSet":
      if (value && value > charSetMax)
        return { [field]: getErrors("charSet", "length") };
      break;
    case "length":
      if (value && value < lngthMin)
        return { [field]: getErrors("length", "min") };
  ...
  switch (field) {
    ...
    case "duration":
      if (value < durationMin)
        return { duration: getErrors("duration", "min") };
      else if (value > durationMax)
        return { duration: getErrors("duration", "max") };
      break;
    case "type_id":
      break;
    default:
