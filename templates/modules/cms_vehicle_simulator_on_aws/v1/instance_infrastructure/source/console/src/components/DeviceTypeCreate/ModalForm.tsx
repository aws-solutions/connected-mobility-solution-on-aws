...
export default function ModalForm(props: IProps): React.JSX.Element {
  ...
  const getAttrFieldDefaults = (type: string) => {
    ...
    return attrFields.reduce((acc: { [key: string]: any }, curr) => {
      let value: any;
      if (curr === "static") {
        value = false;
  ...
  const handleFormChange = (event: any) => {
    ...
    let newFields = {};
    let valueAsNum = isNaN(event.target.valueAsNumber)
      ? null
    ...
    if (attrName === "static") {
      ...
      Array.from(document.querySelectorAll("input")).forEach(
        (input: HTMLInputElement) => {
          if (
            Object.keys(AttributeTypeMap).includes(input.id) &&
            !["name", "type"].includes(input.id)
          ) {
            input.value = "";
          }
        }
      ...
      setShowValidation(
        showValidation.filter((id) => id === "name" || id === "type")
      );
  ...
  return (
    <Modal show={props.showModal} onHide={() => handleModalClose()}>
        ...
        >
          <Form.Group>
            ...
            <Form.Text className="muted">
              {I18n.get("attribute.name.description")}
            </Form.Text>
          </Form.Group>
          <Form.Group>
      ...
      <Modal.Footer>
        <Button className="button-theme-alt" onClick={() => handleModalClose()}>
          {I18n.get("cancel")}
        </Button>
        <Button
          type="submit"
