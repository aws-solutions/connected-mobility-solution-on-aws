...
class GrafanaAlertsConstruct(Construct):
    def __init__(
        self,
        scope: Construct,
        construct_id: str,
        ...
        grafana_alerts_custom_resource_policy = aws_iam.Policy(
            self,
            ...
            statements=[
                aws_iam.PolicyStatement(
                    ...
                    resources=[
                        f"{grafana_s3_bucket_arn}/{EVBatteryHealthConstants.ALERTS_S3_OBJECT_KEY_PREFIX}*",
                    ],
                ),
        ...
        custom_resource_lambda_construct.add_policy_to_custom_resource_lambda(
            policy=grafana_alerts_custom_resource_policy
        )

        create_alerts_upload_to_s3_custom_resource = CustomResource(
            self,
            "create-grafana-alerts-and-upload-to-s3-custom-resource",
            service_token=custom_resource_lambda_construct.custom_resource_lambda.function_arn,
            resource_type=f"Custom::{CustomResourceType.ResourceType.CREATE_GRAFANA_ALERTS_AND_UPLOAD_TO_S3.value}",
            properties={
                ...
                "AlertsS3ObjectKeyPrefix": EVBatteryHealthConstants.ALERTS_S3_OBJECT_KEY_PREFIX,
                "DataSources": data_sources,
            },
        )
        create_alerts_upload_to_s3_custom_resource.node.add_dependency(
            grafana_alerts_custom_resource_policy
        )
