...
def handler(event: Dict[str, Any], context: LambdaContext) -> Dict[str, Any]:
    ...
    try:
        delete_cognito_user_pool_app_client(
            client_id=event["DeleteCognitoUserPoolAppClientInput"]["ClientId"]
        )
        response["Status"] = StatusType.SUCCESS.value
        logger.info("Successfully deleted app client: %s", response)
        ...
        logger.error(
            "Encountered error while deleting App Client!",
            exc_info=True,
        )
...
def delete_cognito_user_pool_app_client(client_id: str) -> None:
    cognito_identity_provider_client = get_cognito_idp_client()

    cognito_identity_provider_client.delete_user_pool_client(
